...
/**
 * @implements LupaExportManagerInterface<TaxonInterface>
 */
class TaxonExportManager implements LupaExportManagerInterface
{
    ...
    public function export(object $object): void
    {
        if (null === $object->getCode()) {
            $this->logger->warning(
                sprintf('Taxon with id %s has no code', $object->getId()),
            );

            return;
        }

        $variants = $this->taxonRepository->findAllEnabledIdsByTaxonCode($object->getCode());
